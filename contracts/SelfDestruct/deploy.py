...
def deploy_attack_contract(client, contract_name, wallet_address, wallet_key, abi, bytecode):
    contract = client.eth.contract(abi=abi, bytecode=bytecode)
    nonce = client.eth.getTransactionCount(wallet_address)
    transaction = contract.constructor().buildTransaction(
        {
...
def main():
    ...
    contracts = ["Game.sol", "AttackGame.sol"]
    ...
    for contract in contracts:
        ...
        if contract == "Game.sol":
            deploy_vuln_contract(ganache_client, contract_name, address_one, address_one_key, contract_abi, contract_bytecode)
        else:
            deploy_attack_contract(ganache_client, contract_name, address_one, address_one_key, contract_abi, contract_bytecode)
